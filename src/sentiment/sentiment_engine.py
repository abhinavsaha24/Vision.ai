...
class SentimentEngine:
    ...
    def get_sentiment(self) -> Dict:
        ...
        # Rate limit updates
        if self._last_result and (time.time() - self._last_update) < self._update_interval:
        ...
        result["rolling_std"] = round(float(np.std(scores)), 4) if len(scores) > 1 else 0.0
        result["momentum"] = round(float(scores[-1] - scores[0]), 4) if len(scores) > 1 else 0.0
