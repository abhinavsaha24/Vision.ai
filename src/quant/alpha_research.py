...
class FactorAnalyzer:
    ...
    def compute_ic_series(self, df: pd.DataFrame, factor_col: str,
                          return_col: str, window: int = 20) -> pd.Series:
    ...
    def analyze_factor(self, df: pd.DataFrame, factor_col: str,
                       return_col: str = "returns") -> Dict:
    ...
    def _quintile_returns(self, df: pd.DataFrame, factor_col: str,
                          return_col: str) -> Dict:
        ...
        valid["quintile"] = pd.qcut(valid[factor_col], q=5, labels=False, duplicates="drop")
...
class CorrelationAnalyzer:
    ...
    def find_redundant_features(self, df: pd.DataFrame, threshold: float = 0.9,
                                feature_cols: Optional[List[str]] = None) -> List[tuple]:
        ...
        for i in range(len(cols)):
            for j in range(i + 1, len(cols)):
                if abs(corr.iloc[i, j]) >= threshold:
                    redundant.append((cols[i], cols[j], round(float(corr.iloc[i, j]), 4)))

        return sorted(redundant, key=lambda x: abs(x[2]), reverse=True)

    def suggest_feature_removal(self, df: pd.DataFrame, threshold: float = 0.9,
                                target_col: str = "returns") -> List[str]:
...
class MutualInformationRanker:
    ...
    def rank_features(self, X: pd.DataFrame, y: pd.Series,
...
class SHAPExplainer:
    ...
    def explain(self, model, X: np.ndarray, feature_names: List[str],
