...
class FeatureSelector:
    ...
    def __init__(self, correlation_threshold: float = 0.95,
                 top_n: int = 30, target_col: str = "Target_Direction"):
    ...
    def remove_redundant(self, df: pd.DataFrame,
            ...
            for j in range(i + 1, len(cols)):
                ...
                if corr_matrix.iloc[i, j] >= self.correlation_threshold:
                    ...
                    if self.target_col in df.columns:
                        corr_i = abs(df[cols[i]].corr(df[self.target_col]))
        ...
        logger.info(f"Redundancy removal: {len(feature_cols)} → {len(kept)} features "
    ...
    def rank_by_mutual_information(self, df: pd.DataFrame,
    ...
    def select(self, df: pd.DataFrame,
               feature_cols: Optional[List[str]] = None) -> List[str]:
        """
        ...
        logger.info(f"Feature selection: {len(feature_cols)} → {len(selected)} features")

        return selected

    def get_feature_report(self, df: pd.DataFrame,
