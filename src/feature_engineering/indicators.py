...
class FeatureEngineer:
    ...
    def add_bollinger(self, df: pd.DataFrame, column="close"):
        ...
        df["BB_pctb"] = _safe_divide(p - df["BB_lower"], df["BB_upper"] - df["BB_lower"], fill=0.5)
    ...
    def add_statistical_features(self, df: pd.DataFrame, window: int = 20) -> pd.DataFrame:
        ...
        ret = df["returns"] if "returns" in df.columns else df["close"].pct_change().fillna(0)
        ...
        for lag in [1, 2, 5]:
            df[f"autocorr_lag{lag}"] = ret.rolling(window, min_periods=lag + 1).apply(
    ...
    def add_microstructure_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        alpha_raw = (np.sqrt(2 * beta) - np.sqrt(beta)) / (3 - 2 * np.sqrt(2)) - np.sqrt(gamma_s / (3 - 2 * np.sqrt(2)))
        alpha_raw = alpha_raw.replace([np.inf, -np.inf], np.nan).fillna(0)
        df["spread_proxy"] = (2 * (np.exp(alpha_raw) - 1) / (1 + np.exp(alpha_raw))).clip(0, 0.1).fillna(0)
        ...
        if "returns" in df.columns:
            df["amihud_illiquidity"] = _safe_divide(df["returns"].abs(), v.replace(0, 1e-8))
        else:
            df["amihud_illiquidity"] = _safe_divide(c.pct_change().abs(), v.replace(0, 1e-8))
    ...
    def add_regime_features(self, df: pd.DataFrame):
        ...
        # Volatility regime
        vol = df["volatility_20"] if "volatility_20" in df.columns else df["close"].pct_change().rolling(20).std()
        ...
        atr = df["ATR"] if "ATR" in df.columns else (df["high"] - df["low"]).rolling(14, min_periods=1).mean()
        ...
        dx = _safe_divide((plus_di - minus_di).abs(), (plus_di + minus_di).replace(0, 1e-8)) * 100
    ...
    def add_multi_horizon_targets(self, df: pd.DataFrame, column="close",
