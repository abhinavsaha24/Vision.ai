...
- `CVE-2026-4539` currently affects `pygments` with no published fixed version in the active package index.
- CI keeps dependency auditing enabled and temporarily ignores only this single CVE (`--ignore-vuln CVE-2026-4539`) until upstream publishes a remediation release.
- Remove the ignore immediately after a fixed `pygments` version is available.
