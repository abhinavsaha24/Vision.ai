...
class StatisticalArbitrage:
    ...
    def test_cointegration(self, series_a: pd.Series, series_b: pd.Series) -> Dict:
        ...
        try:
            ...
            cointegrated = (
                p_value < 0.05 and self.min_half_life <= half_life <= self.max_half_life
            )
            ...
            result = {
                ...
                "critical_values": {
                    k: round(float(v), 4) for k, v in adf_result[4].items()
                },
            }

            logger.info(
                "Cointegration test: p=%.4f, half_life=%.1f, cointegrated=%s",
                p_value,
                half_life,
                cointegrated,
            )
    ...
    def _compute_hedge_ratio(self, series_a: pd.Series, series_b: pd.Series) -> float:
    ...
    def compute_rolling_hedge_ratio(
        self, series_a: pd.Series, series_b: pd.Series, window: int = None
    ) -> pd.Series:
    ...
    def compute_spread(
        self, series_a: pd.Series, series_b: pd.Series, hedge_ratio: float = None
    ) -> pd.Series:
    ...
    def generate_signal(self, series_a: pd.Series, series_b: pd.Series) -> Dict:
        """
        ...
        return {
            ...
            "spread_current": (
                round(float(spread.iloc[-1]), 4) if not spread.empty else 0
            ),
    ...
    def find_pairs(self, price_dict: Dict[str, pd.Series], max_pairs: int = 5) -> list:
        """
