...
class RiskParityStrategy:
    ...
    def __init__(self, vol_target: float = 0.15, vol_lookback: int = 20,
    ...
    def generate_signal(self, df: pd.DataFrame = None, **kwargs) -> int:
        ...
        try:
            ...
            realized_vol = float(returns.iloc[-self.vol_lookback:].std() * np.sqrt(252))
            ...
            logger.error(f"RiskParityStrategy signal error: {e}")
            return 0
    ...
    def inverse_volatility_weights(self, returns_dict: Dict[str, pd.Series],
    ...
    def equal_risk_contribution(self, returns_df: pd.DataFrame,
                                risk_budget: Optional[np.ndarray] = None) -> Dict[str, float]:
        """
        ...
        try:
            ...
            logger.warning(f"ERC optimization error: {e}")
