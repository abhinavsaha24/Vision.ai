...
with st.sidebar:
    ...
    symbol = st.selectbox(
        "Symbol", ["BTCUSDT", "ETHUSDT", "SOLUSDT", "BNBUSDT", "XRPUSDT"]
    )
    ...
    if st.button("🧠 Train Model", use_container_width=True):
        with st.spinner("Training..."):
            ...
            if result:
                st.success(
                    f"✅ Trained — Accuracy: {result.get('metrics', {}).get('accuracy', 'N/A')}"
                )
...
with tab1:
    ...
    if pred:
                ...
                [
                    {
                        "Signal": k,
                        "Value": (
                            "Bullish" if v == 1 else "Bearish" if v == -1 else "Neutral"
                        ),
                    }
        ...
        if preds_list:
            ...
            st.dataframe(
                pd.DataFrame(preds_list), use_container_width=True, hide_index=True
            )
...
with tab2:
    ...
    initial_cap = st.number_input(
        "Initial Capital ($)", value=100000.0, min_value=1000.0
    )

    if st.button("▶️ Run Backtest", use_container_width=True):
        with st.spinner("Running backtest..."):
            result = api_post(
                "/backtest/run", {"symbol": symbol, "initial_capital": initial_cap}
            )
...
with tab4:
    ...
    if sentiment:
        ...
        if details:
            ...
            st.dataframe(
                pd.DataFrame(details[:15]), use_container_width=True, hide_index=True
            )
    else:
...
with tab5:
    ...
    if risk:
        ...
        if events:
            ...
            st.dataframe(
                pd.DataFrame(events[-10:]), use_container_width=True, hide_index=True
            )
    else:
...
with tab6:
    ...
    if strategies and "strategies" in strategies:
        ...
        st.dataframe(
            pd.DataFrame(strategies["strategies"]),
            use_container_width=True,
            hide_index=True,
        )
