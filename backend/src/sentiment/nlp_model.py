...
class FinBERTSentimentModel:
    ...
    def analyze(self, texts: List[str]) -> Dict:
        ...
        return {
            ...
            "label": "bullish" if aggregate_score > 0.1 else "bearish" if aggregate_score < -0.1 else "neutral",
            "count": len(texts),
            "details": details,
        }
...
class RuleSentimentModel:
    ...
    BULLISH_WORDS = {
        ...
        "profit", "gain", "rise", "up", "growth", "positive", "higher",
        "buy", "long", "support", "recover", "bounce", "strong",
        "outperform", "upgrade", "beat", "exceed", "record",
    }

    BEARISH_WORDS = {
        "bear", "bearish", "crash", "dump", "sell", "selloff", "plunge",
        "loss", "drop", "fall", "down", "decline", "negative", "lower",
        "short", "resistance", "weak", "fail", "miss", "downgrade",
        "risk", "fear", "panic", "liquidation", "bankrupt",
    }
...
def create_sentiment_model():
    ...
    if _HAS_TRANSFORMERS:
        try:
            ...
            logger.warning(f"FinBERT failed to load: {e} — using rule-based fallback")
