...
class LiveTradingSafety:
    ...
    def run_preflight(self) -> PreFlightReport:
        ...
        if all_passed:
            ...
            in_live_mode = (
                bool(getattr(self.settings, "live_trading_enabled", False))
                and str(getattr(self.settings, "trading_mode", "paper")).lower() == "live"
            )
            if in_live_mode:
                logger.warning(
                    "❌ Live trading pre-flight BLOCKED: %s check(s) failed — %s",
                    len(blocked),
                    blocked,
                )
            else:
                logger.info(
                    "Live trading pre-flight blocked in paper mode (expected): %s",
                    blocked,
                )
    ...
    def _check_exchange_connectivity(self) -> SafetyCheckResult:
        ...
        try:
            ...
            totals = getattr(balance, "total", {}) or {}
            usdt = float(totals.get("USDT", 0.0) or 0.0)
            assets_with_balance = [
                k for k, v in totals.items() if float(v or 0.0) > 0.0
            ]

            # Empty balances usually indicate auth/connectivity failure was swallowed upstream.
            if not totals:
                return SafetyCheckResult(
                    name="exchange_connectivity",
                    passed=False,
                    message="Exchange connection failed: balance payload empty",
                )

            if usdt <= 0.0 and not assets_with_balance:
                return SafetyCheckResult(
                    name="exchange_connectivity",
                    passed=False,
                    message="Exchange connected but no available balances detected",
                )
