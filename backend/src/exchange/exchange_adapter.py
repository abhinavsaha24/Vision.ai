...
class Order:
    ...
    status: str = (
        "pending"  # pending → submitted → partial → filled | cancelled | rejected
    )
...
class PaperAdapter(ExchangeAdapter):
    ...
    def place_market_order(
        ...
        if side == "buy":
            ...
            # Allow short selling — holdings can go negative (simulated short)
        ...
        logger.info(
            "[PAPER] %s MARKET %.6f %s @ %.2f (slip=%.2f)",
            side.upper(),
            quantity,
            symbol,
            fill_price,
...
class BinanceAdapter(ExchangeAdapter):
    ...
    def __init__(self, api_key: str, secret: str, testnet: bool = False):
        ...
        logger.info(
            "BinanceAdapter initialized (testnet=%s, rate_limit=%s)",
            testnet,
            self.exchange.enableRateLimit,
        )
    ...
    def _ccxt_to_order(self, raw: Dict, symbol: str = "") -> Order:
        ...
        return Order(
            ...
            commission=float(
                raw.get("fee", {}).get("cost", 0) if raw.get("fee") else 0
            ),
    ...
    def place_market_order(
        ...
        try:
            ...
            logger.error("[LIVE] Market order failed: %s", e)
            return Order(
                order_id="",
                symbol=symbol,
                side=side,
    ...
    def place_limit_order(
        ...
        try:
            ...
            logger.error("[LIVE] Limit order failed: %s", e)
            return Order(
                order_id="",
                symbol=symbol,
                side=side,
    ...
    def cancel_order(self, order_id: str, symbol: str) -> Order:
        try:
            ...
            logger.error("[LIVE] Cancel failed: %s", e)
            return Order(
                order_id=order_id,
                symbol=symbol,
                side="",
    ...
    def get_order_status(self, order_id: str, symbol: str) -> Order:
        try:
            ...
            logger.error("[LIVE] Order status check failed: %s", e)
    ...
    def get_balance(self) -> Balance:
        try:
            ...
            logger.error("[LIVE] Balance fetch failed: %s", e)
            return Balance()
    ...
    def get_open_orders(self, symbol: str = "") -> List[Order]:
        try:
            ...
            logger.error("[LIVE] Open orders fetch failed: %s", e)
            return []
