...
_SERVICE_DELEGATION_STATE: Dict[str, Dict[str, Any]] = {}
_LOCAL_MESH_HOSTS = {
    "risk-service",
    "portfolio-service",
    "execution-service",
...
def _is_service_mesh_host(service_base_url: str) -> bool:
    host = (urlparse(service_base_url).hostname or "").strip().lower()
    return bool(host) and (host in _LOCAL_MESH_HOSTS or host.endswith("-service"))


def _delegation_key(service_base_url: str, endpoint_path: str) -> str:
    return f"{service_base_url.rstrip('/')}{endpoint_path}"


def _delegation_cooldown_seconds(failures: int) -> float:
    base = max(float(settings.internal_service_failure_cooldown_seconds), 1.0)
    maximum = max(float(settings.internal_service_failure_cooldown_max_seconds), base)
    return min(base * (2 ** max(failures - 1, 0)), maximum)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        svc.realtime_feed = RealtimeMarketFeed(
            ...
            ws_enabled=settings.realtime_ws_enabled,
            ws_open_timeout_seconds=settings.realtime_ws_open_timeout_seconds,
            ws_max_consecutive_failures=settings.realtime_ws_max_consecutive_failures,
            ws_cooldown_seconds=settings.realtime_ws_cooldown_seconds,
            rest_poll_seconds=settings.realtime_rest_poll_seconds,
        )
...
docs_enabled = bool(getattr(settings, "enable_openapi_docs", False))
...
cors_allow_origin_regex = raw_cors_regex or r"https://visiontrading(-[a-z0-9]+)?\.vercel\.app"
cors_allow_methods = [
    method.strip().upper()
    for method in str(getattr(settings, "cors_allow_methods", "") or "").split(",")
    if method.strip()
]
...
cors_allow_headers = [
    header.strip()
    for header in str(getattr(settings, "cors_allow_headers", "") or "").split(",")
    if header.strip()
]
...
class StrategyControlRequest(BaseModel):
    strategy_name: str = Field(default="alpha_model", min_length=1, max_length=64)
...
def _service_get_json(
    ...
    if not service_base_url or not settings.internal_service_delegation_enabled:
        return None

    request_host = (request.url.hostname or "").strip().lower()
    if (
        settings.internal_service_fail_fast_local_hosts
        and request_host in {"localhost", "127.0.0.1"}
        and _is_service_mesh_host(service_base_url)
    ):
        # Local monolith runs should skip mesh-style host delegation.
        return None

    key = _delegation_key(service_base_url, endpoint_path)
    now = time.monotonic()
    state = _SERVICE_DELEGATION_STATE.get(
        key,
        {"failures": 0, "blocked_until": 0.0, "warn_after": 0.0},
    )
    ...
    try:
        ...
        timeout_seconds = max(float(settings.internal_service_timeout_seconds), 0.25)
        connect_timeout = min(0.75, timeout_seconds)
        ...
        _SERVICE_DELEGATION_STATE.pop(key, None)
        return response.json()
    except Exception as e:
        failures = int(state.get("failures", 0)) + 1
        cooldown = _delegation_cooldown_seconds(failures)
        blocked_until = now + cooldown
        warn_after = float(state.get("warn_after", 0.0) or 0.0)
        _SERVICE_DELEGATION_STATE[key] = {
            "failures": failures,
            "blocked_until": blocked_until,
            "warn_after": blocked_until,
        }

        if now >= warn_after:
            logger.warning(
                "Service delegation failed for %s (failures=%s, cooldown=%.1fs): %s",
                endpoint_path,
                failures,
                cooldown,
                e,
...
def _strategy_control_cache_key(strategy_name: str) -> str:
    normalized = str(strategy_name or "alpha_model").strip().lower() or "alpha_model"
    return f"strategy:control:{normalized}"


def _is_strategy_enabled(svc: AppServices, strategy_name: str) -> bool:
    if svc.cache is None:
        return True
    state = svc.cache.get_json(_strategy_control_cache_key(strategy_name))
    if isinstance(state, dict) and "enabled" in state:
        return bool(state.get("enabled"))
    return True


def _set_strategy_enabled(svc: AppServices, strategy_name: str, enabled: bool) -> None:
    if svc.cache is None:
        return
    payload = {
    ...
    svc.cache.set_json(_strategy_control_cache_key(strategy_name), payload, ttl=7 * 24 * 3600)
...
@app.post("/data/fetch", dependencies=[Depends(get_current_user)])
...
@app.get("/market/realtime", dependencies=[Depends(get_current_user)])
...
@app.get("/market/history", dependencies=[Depends(get_current_user)])
...
@app.post("/features/generate", dependencies=[Depends(get_current_user)])
...
async def list_strategies(request: Request):
    ...
    strategy_key = "alpha_model"
    return {
        "strategies": [
            {
                "name": "Alpha Model",
                "key": strategy_key,
                "active": _is_strategy_enabled(svc, strategy_key),
                "weight": 1.0,
                "type": "model_driven",
                "thresholds": {
...
@app.post("/strategy/start", dependencies=[Depends(require_admin)])
async def start_strategy_control(payload: StrategyControlRequest, request: Request):
    svc = _svc(request)
    strategy_name = str(payload.strategy_name).strip() or "alpha_model"
    _set_strategy_enabled(svc, strategy_name, True)
    return {
        "status": "applied",
...
@app.post("/strategy/stop", dependencies=[Depends(require_admin)])
async def stop_strategy_control(payload: StrategyControlRequest, request: Request):
    svc = _svc(request)
    strategy_name = str(payload.strategy_name).strip() or "alpha_model"
    _set_strategy_enabled(svc, strategy_name, False)
    return {
        "status": "applied",
        "strategy_name": strategy_name,
        "enabled": False,
...
@app.get("/strategy/status/{strategy_name}", dependencies=[Depends(get_current_user)])
async def strategy_control_status(strategy_name: str, request: Request):
    svc = _svc(request)
    normalized = str(strategy_name).strip() or "alpha_model"
    return {
        "strategy_name": normalized,
        "enabled": _is_strategy_enabled(svc, normalized),
    }
...
@app.get("/system/readiness", dependencies=[Depends(get_current_user)])
...
def _validate_ws_origin(websocket: WebSocket) -> bool:
    """Validate WebSocket handshake origin against CORS allowlist."""
    origin = (websocket.headers.get("origin") or "").strip()
    if not origin:
        # Allow connections without Origin header only when configured.
        return not bool(getattr(settings, "ws_require_origin_header", False))
    ...
    import re
    try:
        if re.fullmatch(cors_allow_origin_regex, origin):
            return True
    except re.error:
        pass
    return False
...
def _extract_ws_token(websocket: WebSocket) -> Optional[str]:
    """Extract auth token from header/subprotocol/cookie with query fallback."""
    ...
    protocols_header = websocket.headers.get("sec-websocket-protocol") or ""
    ...
    cookie_token = (websocket.cookies.get("vision_ai_token") or "").strip()
    if cookie_token:
        return cookie_token
...
def _select_ws_subprotocol(websocket: WebSocket) -> Optional[str]:
    """Select a supported websocket subprotocol when offered by client."""
    protocols_header = websocket.headers.get("sec-websocket-protocol") or ""
    offered = {token.strip() for token in protocols_header.split(",") if token.strip()}
    if "vision-ai.v1" in offered:
        return "vision-ai.v1"
    return None
...
async def _stream_channel(
    ...
    # -- WebSocket Authentication via Authorization/subprotocol/cookie --
    token = _extract_ws_token(websocket)
    if not _validate_ws_token(token):
        await websocket.close(code=4001, reason="Authentication required")
        return

    selected_subprotocol = _select_ws_subprotocol(websocket)
...
@app.get("/market-intelligence", dependencies=[Depends(get_current_user)])
async def market_intelligence(request: Request):
