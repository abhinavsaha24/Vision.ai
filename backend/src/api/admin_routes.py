...
async def disable_user(
    ...
    try:
        ...
        cursor.execute("SELECT id, email FROM users WHERE id=?", (req.user_id,))
        ...
        cursor.execute(
            "UPDATE users SET is_active=0 WHERE id=?", (req.user_id,)
        )
        conn.commit()
        conn.close()

        logger.info(f"Admin {admin.get('user_id')} disabled user {req.user_id}: {req.reason}")
