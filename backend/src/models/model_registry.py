...
class ModelRegistry:
    ...
    def _load_registry(self):
        ...
        if self.registry_path.exists():
            try:
                ...
                logger.info(
                    "Model registry loaded: %s versions, active=%s",
                    len(self.versions),
                    self.active_version,
                )
    ...
    def register_model(
        ...
        self._save_registry()
        logger.info("Model registered: v%s (accuracy=%.4f)", version_id, version.accuracy)

        return version

    def should_rollback(self, new_metrics: Dict, threshold: float = 0.02) -> bool:
        """
        ...
        if new_acc < old_acc - threshold:
            logger.warning(
                "Model regression detected: new=%.4f < current=%.4f (threshold=%.4f)",
                new_acc,
                old_acc,
                threshold,
            )
    ...
    def rollback(self) -> Optional[ModelVersion]:
        ...
        self._save_registry()
        logger.info("Rolled back to v%s", previous.version_id)
        return previous
