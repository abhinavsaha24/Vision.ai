...
class MarketRegimeDetector:
    ...
    def fit_hmm(self, returns: np.ndarray) -> bool:
        ...
        try:
            ...
            logger.info(
                "HMM fitted: %s regimes, means=%s, vars=%s",
                self.n_regimes,
                [round(float(m), 6) for m in means],
                [round(float(v), 6) for v in covs],
            )
            return True
    ...
    def detect_structure(self, df: pd.DataFrame) -> str:
        """Classify structural state as TREND, RANGE, or VOLATILE."""
        ...
        ema_short = float(df["EMA_12"].iloc[-1]) if "EMA_12" in df.columns else close
        ema_long = float(df["EMA_26"].iloc[-1]) if "EMA_26" in df.columns else close
        ema_spread = abs(ema_short - ema_long) / max(close, 1e-8)

        vol_state = self.detect_volatility(df)
        if vol_state == "high_volatility":
            return "VOLATILE"

        if adx >= 24 and ema_spread >= 0.004:
            return "TREND"
    ...
    def get_regime(self, df: pd.DataFrame) -> Dict:
        ...
        market_state = self.detect_structure(df)
        ...
        elif trend == "downtrend" and volatility in (
            "high_volatility",
            "elevated_volatility",
        ):
            regime_label = "crisis"
    ...
    def should_allow_entries(self, regime: Dict, min_confidence: float = 0.6) -> bool:
        ...
        if regime.get("market_state") == "VOLATILE":
            return False
