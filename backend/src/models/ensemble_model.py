...
class EnsembleModel:
    ...
    def __init__(
        self, models: Dict[str, object], weights: Optional[Dict[str, float]] = None
    ):
        """
        Args:
    ...
    def predict_structured(
        self, X, volatility_data: Optional[np.ndarray] = None
    ) -> list:
        """
        ...
        predictions = []
        for i, prob_val in enumerate(ensemble_prob):
            prob_up = float(prob_val)
            ...
            if len(model_preds_i) > 1:
                agreement = (
                    1.0 - float(np.std(model_preds_i)) * 2
                )  # 0=disagree, 1=agree
