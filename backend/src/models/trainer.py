...
from backend.src.models.lstm_model import PyTorchLSTMModel
...
class ModelTrainer:
    ...
    def __init__(self, model_dir: str = "models", test_size: float = 0.2):
        ...
        self.weights = {"rf": 0.25, "xgb": 0.35, "lgb": 0.25, "lstm": 0.15}
    ...
    def train(
        ...
        if run_cv and len(X) > n_splits * 10:
            ...
            for fold, (train_idx, val_idx) in enumerate(tscv.split(X)):
                ...
                logger.info("CV Fold %s: accuracy=%.4f", fold + 1, score)
        ...
        try:
            ...
            self.lstm = PyTorchLSTMModel(
                input_size=X_train_s.shape[1], seq_len=10, epochs=10
            )
            self.lstm.fit(X_train_s, y_train)
        except Exception as e:
            logger.error("Failed to train LSTM: %s", e)
            self.lstm = None
        ...
        if len(X_test_s) > 0:
            ...
            logger.info("Test accuracy (ensemble): %.4f", accuracy)
    ...
    def predict_proba_ensemble(self, X_scaled: np.ndarray) -> np.ndarray:
        ...
        if self.lstm and self.weights.get("lstm", 0) > 0:
            lstm_prob = self.lstm.predict_proba(X_scaled)[:, 1]
            combined += lstm_prob * self.weights["lstm"]
    ...
    def _compute_feature_importance(self):
        """Aggregate feature importance from all tree models."""
        importance = {}

        # RF importance
        rf_imp = getattr(self.rf, "feature_importances_", None)
        if rf_imp is not None:
            for i, name in enumerate(self.feature_names_):
                importance[name] = float(rf_imp[i])

        # XGBoost importance
        xgb_imp = getattr(self.xgb, "feature_importances_", None)
        if xgb_imp is not None:
            for i, name in enumerate(self.feature_names_):
                importance[name] = (importance.get(name, 0) + float(xgb_imp[i])) / 2

        # LightGBM importance
        lgb_imp = getattr(self.lgb, "feature_importances_", None)
        if lgb_imp is not None:
            lgb_total = lgb_imp.sum() if lgb_imp.sum() > 0 else 1
            for i, name in enumerate(self.feature_names_):
                importance[name] = (
                    importance.get(name, 0) * 2 + float(lgb_imp[i] / lgb_total)
                ) / 3
        ...
        self.metrics["top_features"] = list(self.metadata.feature_importances.keys())[
            :15
        ]
    ...
    def save(self, name: str = "trading_model"):
        ...
        if self.lstm and getattr(self.lstm, "_is_fitted", False):
            torch.save(
                self.lstm.model.state_dict(), self.model_dir / f"{name}_lstm.pth"
            )
            joblib.dump(
                {"seq_len": self.lstm.seq_len, "input_size": self.feature_names_},
                self.model_dir / f"{name}_lstm_meta.joblib",
            )

        logger.info("Models saved: %s", name)
    ...
    def load(self, name: str = "trading_model"):
        ...
        # Try loading PyTorch LSTM
        lstm_path = self.model_dir / f"{name}_lstm.pth"
        if lstm_path.exists():
            try:
                lstm_meta = joblib.load(self.model_dir / f"{name}_lstm_meta.joblib")
                self.lstm = PyTorchLSTMModel(
                    input_size=len(lstm_meta["input_size"]),
                    seq_len=lstm_meta["seq_len"],
                )
                self.lstm.model.load_state_dict(
                    torch.load(lstm_path, map_location=self.lstm.device)
                )
                self.lstm._is_fitted = True
            except Exception as e:
                logger.warning("Failed to load LSTM from %s: %s", lstm_path, e)
                self.lstm = None

        # Backward compatibility
        if hasattr(self, "feature_names") and not self.feature_names_:
            self.feature_names_ = getattr(self, "feature_names")

        logger.info("Models loaded: %s", name)
