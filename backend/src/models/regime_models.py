...
def _label_regimes(
    means: np.ndarray, covars: np.ndarray, n_states: int
) -> Dict[int, str]:
    """Assign descriptive labels based on state means/variances."""
    labels = {}
    vols = (
        np.sqrt(covars.flatten()[:n_states])
        if covars.ndim > 1
        else np.sqrt(covars[:n_states])
    )
...
class HMMRegimeDetector:
    ...
    def __init__(self, n_states: int = 3, n_iter: int = 100, random_state: int = 42):
        if not _HAS_HMM:
            raise ImportError(
                "hmmlearn is required for HMMRegimeDetector. Install: pip install hmmlearn"
            )
    ...
    def fit(self, df: pd.DataFrame) -> "HMMRegimeDetector":
        ...
        logger.info("HMM fitted — {self.n_states} states, labels: %s", self.labels)
        return self
...
class GMMRegimeDetector:
    ...
    def fit(self, df: pd.DataFrame) -> "GMMRegimeDetector":
        ...
        logger.info("GMM fitted — {self.n_components} components, labels: %s", self.labels)
        return self
