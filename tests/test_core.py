...
class TestPaperAdapter:
    ...
    def test_short_selling_supported(self):
        """Verify that PaperAdapter now supports short selling (sell without holdings)."""
        adapter = PaperAdapter(initial_cash=10000)
        # Sell 1.0 BTC without owning any — should be allowed (short)
        order = adapter.place_market_order("BTC/USDT", "sell", 1.0, price=50000)

        assert order.status == "filled"
        assert order.side == "sell"
        assert adapter.get_balance().total["BTC/USDT"] == -1.0
